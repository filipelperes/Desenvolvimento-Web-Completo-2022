...
class Despesa {
   ...
   validarDados() {
      for(let i in this) if (this[i] == undefined || this[i] == null || this[i] == '') return false
      return true
   }
}
...
class BD {
   ...
   getRegisters() {
      let despesas = [], id = localStorage.getItem('id');
      for(let i = 1; i <= id; i++)
...
const loadDespesas = () => {
   let despesas = bd.getRegisters();
   renderDespesas(despesas);
}

const searchD = () => {
   let ano = document.getElementById('ano').value, mes = document.getElementById('mes').value, dia = document.getElementById('dia').value, tipo = document.getElementById('tipo').value, descricao = document.getElementById('descricao').value, valor = document.getElementById('valor').value;
   let despesa = new Despesa(ano, mes, dia, tipo, descricao, valor);
...
const renderDespesas = (despesas) => {
   let tbody = document.querySelectorAll('table.table tbody');
   tbody.innerHTML = '';
   despesas.forEach(d => {
